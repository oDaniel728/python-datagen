...
from datagenpp.extras.item.bundle import Bundle
...
class ItemBundle(Item):
    def __init__(self, items: list[ItemStack], name: BaseText = LiteralText("Bundle"), rarity: BaseItemSettings._TRarity = "common") -> None:
        ...
        item_settings = BaseItemSettings().with_item_name(name).with_rarity(rarity).with_custom_data({"bundle": True, "show": True})
